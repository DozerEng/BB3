/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2022 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32g4xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

void HAL_TIM_MspPostInit(TIM_HandleTypeDef *htim);

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define LED2_B_Pin GPIO_PIN_13
#define LED2_B_GPIO_Port GPIOC
#define LED2_G_Pin GPIO_PIN_14
#define LED2_G_GPIO_Port GPIOC
#define LED2_R_Pin GPIO_PIN_15
#define LED2_R_GPIO_Port GPIOC
#define nRST_Pin GPIO_PIN_10
#define nRST_GPIO_Port GPIOG
#define FDCAN1_FAULT_Pin GPIO_PIN_0
#define FDCAN1_FAULT_GPIO_Port GPIOC
#define FDCAN1_S_Pin GPIO_PIN_1
#define FDCAN1_S_GPIO_Port GPIOC
#define STATUS_LED1_Pin GPIO_PIN_2
#define STATUS_LED1_GPIO_Port GPIOC
#define STATUS_LED2_Pin GPIO_PIN_3
#define STATUS_LED2_GPIO_Port GPIOC
#define RIGHT_DIR_Pin GPIO_PIN_0
#define RIGHT_DIR_GPIO_Port GPIOA
#define RIGHT_STEP_Pin GPIO_PIN_1
#define RIGHT_STEP_GPIO_Port GPIOA
#define DEBUG_TX_Pin GPIO_PIN_2
#define DEBUG_TX_GPIO_Port GPIOA
#define DEBUG_RX_Pin GPIO_PIN_3
#define DEBUG_RX_GPIO_Port GPIOA
#define LEFT_STEP_Pin GPIO_PIN_5
#define LEFT_STEP_GPIO_Port GPIOA
#define LEFT_DIR_Pin GPIO_PIN_6
#define LEFT_DIR_GPIO_Port GPIOA
#define BAT_MON_Pin GPIO_PIN_7
#define BAT_MON_GPIO_Port GPIOA
#define STEPPER_TX_Pin GPIO_PIN_4
#define STEPPER_TX_GPIO_Port GPIOC
#define STEPPER_RX_Pin GPIO_PIN_5
#define STEPPER_RX_GPIO_Port GPIOC
#define LIMIT_SW_Pin GPIO_PIN_0
#define LIMIT_SW_GPIO_Port GPIOB
#define TOP_PB_Pin GPIO_PIN_1
#define TOP_PB_GPIO_Port GPIOB
#define MID_PB_Pin GPIO_PIN_2
#define MID_PB_GPIO_Port GPIOB
#define BOT_PB_Pin GPIO_PIN_10
#define BOT_PB_GPIO_Port GPIOB
#define IMU_INTERRUPT_Pin GPIO_PIN_11
#define IMU_INTERRUPT_GPIO_Port GPIOB
#define PWM6_Pin GPIO_PIN_6
#define PWM6_GPIO_Port GPIOC
#define PWM5_Pin GPIO_PIN_7
#define PWM5_GPIO_Port GPIOC
#define PWM4_Pin GPIO_PIN_8
#define PWM4_GPIO_Port GPIOC
#define PWM3_Pin GPIO_PIN_9
#define PWM3_GPIO_Port GPIOC
#define PWM2_Pin GPIO_PIN_8
#define PWM2_GPIO_Port GPIOA
#define PWM1_Pin GPIO_PIN_9
#define PWM1_GPIO_Port GPIOA
#define LED1_R_Pin GPIO_PIN_10
#define LED1_R_GPIO_Port GPIOC
#define LED1_G_Pin GPIO_PIN_11
#define LED1_G_GPIO_Port GPIOC
#define LED1_B_Pin GPIO_PIN_12
#define LED1_B_GPIO_Port GPIOC
#define FDCAN2_FAULT_Pin GPIO_PIN_4
#define FDCAN2_FAULT_GPIO_Port GPIOB
#define FDCAN2_S_Pin GPIO_PIN_7
#define FDCAN2_S_GPIO_Port GPIOB

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
